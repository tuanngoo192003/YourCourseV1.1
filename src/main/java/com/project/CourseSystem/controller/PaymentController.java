...
public class PaymentController {
    ...
    SystemAccountConverter system_accountConverter;
    ...
    @PostMapping("/cart")
    public String cart(@ModelAttribute("courseDTO") CourseDTO courseDTO,
            @RequestParam("paymentButton") String paymentButton,
            @RequestParam("fromPage") String fromPage, Model model, HttpServletRequest request,
    ...
    public String paymentForPage(@ModelAttribute("courseDTO") CourseDTO courseDTO,
            @RequestParam("paymentButton") String paymentButton,
            @RequestParam("fromPage") String fromPage, Model model, HttpServletRequest request,
            HttpServletResponse response) {
    ...
    @GetMapping("/checkout")
    public String checkout(@RequestParam("cartButton") String cartButton, Model model, HttpServletRequest request,
            HttpServletResponse response) {
        HttpSession session = request.getSession();
        if (session.getAttribute("CSys") == null) {
            return authController.loginPage(model, request, response);
    ...
    public String checkoutConfirm(@ModelAttribute("paymentInfo") Payment payment,
            @RequestParam("oneOrMany") String oneOrMany,
            @RequestParam("paymentStatus") String paymentStatus, Model model, HttpServletRequest request,
            HttpServletResponse response) {
    ...
    public String checkoutConfirmForOne(Payment payment, CourseDTO courseDTOTemp, Model model,
        ...
        session.setAttribute("successMsg",
                "Checkout successfully! Waiting for admin to confirm your payment! You can check your payment status in your profile!");
        return "redirect:/course";
    }

    public String checkoutConfirmForMany(Payment payment, Model model, HttpServletRequest request,
            HttpServletResponse response) {
        HttpSession session = request.getSession();
    ...
    public String confirmPayment(@RequestParam("paymentID") String paymentID,
            @RequestParam("pStatus") String paymentStatus,
        ...
        if (paymentStatus.equals("Confirm")) {
            ...
            List<PaymentDetails> paymentDetailsList = paymentDetailsService
                    .getAllPaymentDetailsByPaymentID(payment.getPaymentID());
            // enroll user to course
            for (int i = 0; i < paymentDetailsList.size(); i++) {
                Enrolled enrolled = new Enrolled();
                CourseDTO courseDTO = courseService
                        .getCourseByID(paymentDetailsList.get(i).getCourseID().getCourseID());
