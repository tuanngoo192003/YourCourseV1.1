...
public class CourseController {
    ...
    final private SystemAccountConverter system_accountConverter;
    ...
    public CourseController(CourseService courseService, CategoryService categoryService,
            ...
            SystemAccountConverter system_accountConverter, GoogleDriveService driveService,
    ...
    public String getPaginated(@PathVariable(value = "pageNo") int pageNo,
        ...
        if (session.getAttribute("CSys") != null) {
            ...
            SystemAccount account = system_accountConverter
                    .convertDTOToEntity(accountService.findUserByAccountName(accountName));
        ...
        if (!discountList.isEmpty())
            model.addAttribute("listOfDiscount", discountList);
    ...
    public String myCourseFilter(@ModelAttribute("category") CategoryDTO categoryDTO, Model model,
        ...
        if (categoryDTO.getCategoryName().equals("All")) {
            ...
            return getMyCoursePaginatedByAttribute(1, "courseID", "desc", "categoryID", String.valueOf(categoryID),
    ...
    public String filter(@ModelAttribute("category") CategoryDTO categoryDTO, Model model,
        ...
        if (categoryDTO.getCategoryName().equals("All")) {
            ...
            int categoryID = temp.getCategoryID();
            model.addAttribute("courseList", courseService.getAllCoursesByCategoryID(categoryID));
            return getPaginatedByAttribute(1, "courseID", "desc", "categoryID", String.valueOf(categoryID), model,
                    request, response);
        }
    }
    ...
    public String getCourseByCategoryID(@RequestParam Integer categoryID, Model model,
        ...
        model.addAttribute("category", categoryService.getAllCategories());
        model.addAttribute("courseList", courseService.getAllCoursesByCategoryID(categoryID));
        return getPaginatedByAttribute(1, "courseID", "desc", "categoryID", String.valueOf(categoryID), model, request,
                response);
    }
    ...
    public String addLesson(@RequestParam("file") MultipartFile file,
        ...
        course.setCategoryID(
                categoryConverter.convertDtoToEtity(categoryService.getCategoryByName(addCourseForm.getCategory())));
        ...
        try {
            ...
            com.google.api.services.drive.model.File file1 = driveService.uploadFile(tempFile.getName(),
                    tempFile.getAbsolutePath(), "image/jpg", "CourseAvatar");
    ...
    public String createNewCourse(@RequestParam("questionDTO-content") String questionContents,
        ...
        if (submit.equals("Add new lesson")) {
            ...
            if (quizListFormList == null) {
                ...
                Quiz quiz = saveQuizToDatabase(questionContents, answerContents, quizListForm, course, model, request,
                        response);

                // Save lesson
                List<AddLessonFormDTO> addLessonFormDTOList = (List<AddLessonFormDTO>) session
                        .getAttribute("addLessonFormDTOList");
                ...
                saveCurrentQuestionsAndAnswers(questionContents, answerContents, quizListForm, model, request,
                        response);
                List<AddLessonFormDTO> addLessonFormDTOList = (List<AddLessonFormDTO>) session
                        .getAttribute("addLessonFormDTOList");
                ...
                for (int start = 0; start < addLessonFormDTOList.size(); start++) {
                    ...
                    Quiz quiz = saveQuizToDatabase(questionContentsTemp, answerContentsTemp, quizListFormTemp, course,
                            model, request, response);
    ...
    private void saveCurrentQuestionsAndAnswers(String questionContents, String answerContents,
    ...
    private Quiz saveQuizToDatabase(String questionContents, String answerContents, QuizListForm quizListForm,
        ...
        for (int i = 0; i < answerContents.length(); i++) {
            ...
            if (answerContents.charAt(i) == ',' || i == answerContents.length() - 1) {
                ...
                if (ordinalCount == 0) {
                    answerDTO.setAnswerOrdinal("optionA");
                    answerOrdinal = "optionA";
                    ordinalCount = 1;
                } else if (ordinalCount == 1) {
                    answerDTO.setAnswerOrdinal("optionB");
                    answerOrdinal = "optionB";
                    ordinalCount = 2;
                } else if (ordinalCount == 2) {
                    answerDTO.setAnswerOrdinal("optionC");
                    answerOrdinal = "optionC";
                    ordinalCount = 3;
                } else if (ordinalCount == 3) {
                    answerDTO.setAnswerOrdinal("optionD");
                    answerOrdinal = "optionD";
                    ordinalCount = 0;
                }
                if (tempIndex == 0) {
